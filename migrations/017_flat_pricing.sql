...
--
-- Replaces the old 4-tier model (trial/starter/pro/business). The old
-- CHECK constraint on customers.plan would reject the new value, so it is
-- dropped (same class of issue as the priority constraint). Trial is now a
-- STATUS, not a plan: every account is on the single 'standard' plan; a
...
-- 1. Drop the old tier CHECK constraints (auto-named variants too).
...
-- 3. Re-pin the plan default and normalize every existing row onto the
--    single plan. Keep any in-flight trial window; default a fresh 14-day
--    trial where one isn't set.
